...
func (c *Controller) List(ctx *gin.Context) {
	...
	if err != nil {
		...
		ctx.JSON(http.StatusInternalServerError, dto.StandardResponse{
			Success: false,
			Error:   errResp("INTERNAL_ERROR", "An error occurred while fetching operation centers"),
		})
		return
	}

	response := make([]dto.OperationCenterResponse, 0, len(items))
...
func (c *Controller) GetByID(ctx *gin.Context) {
	...
	if err != nil {
		writeError(ctx, err, "An error occurred while fetching the operation center")
		return
	}
...
func (c *Controller) Create(ctx *gin.Context) {
	...
	if err != nil {
		...
		ctx.JSON(http.StatusInternalServerError, dto.StandardResponse{
			Success: false,
			Error:   errResp("INTERNAL_ERROR", "An error occurred while creating the operation center"),
		})
		return
	}
...
func (c *Controller) Update(ctx *gin.Context) {
	...
	if err != nil {
		writeError(ctx, err, "An error occurred while updating the operation center")
		return
	}
	ctx.JSON(http.StatusOK, dto.StandardResponse{Success: true, Data: mapper.ToResponse(*item)})
}
...
func (c *Controller) Delete(ctx *gin.Context) {
	...
	if err := c.service.Delete(ctx.Request.Context(), id); err != nil {
		writeError(ctx, err, "An error occurred while deleting the operation center")
		return
	}
...
func parseID(ctx *gin.Context) (int64, bool) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, dto.StandardResponse{
			Success: false,
			Error:   errResp("INVALID_ID", "Invalid operation center ID"),
		})
		return 0, false
	}
	return id, true
...
func writeError(ctx *gin.Context, err error, internalMessage string) {
	...
	case errors.Is(err, entity.ErrNotFound):
		ctx.JSON(http.StatusNotFound, dto.StandardResponse{
			Success: false,
			Error:   errResp("NOT_FOUND", "Operation center not found"),
		})
	default:
		log.Printf("Operation center request failed: %v", err)
		ctx.JSON(http.StatusInternalServerError, dto.StandardResponse{
			Success: false,
